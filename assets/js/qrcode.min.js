/*
 * qrcode-generator (minimal subset) — vendored vanilla JS, no deps.
 * Based on the public-domain implementation by Kazuhiko Arase
 * (https://github.com/kazuhikoarase/qrcode-generator). Trimmed to support
 * createDataURL(cellSize, margin) which is all this project needs.
 * License: MIT
 */
var qrcode = function () {
  ...
  function qrPolynomial(num, shift) {
    ...
    var _num = (function () {
      ...
      for (var i = 0; i < num.length - offset; i += 1) n[i] = num[i + offset];
      return n;
    })();
    var _this = {};
    ...
    _this.multiply = function (e) {
      ...
      for (var i = 0; i < n.length; i += 1) n[i] = 0;
      for (var i = 0; i < _this.getLength(); i += 1) {
        for (var j = 0; j < e.getLength(); j += 1) {
          n[i + j] ^= QRMath.gexp(QRMath.glog(_this.getAt(i)) + QRMath.glog(e.getAt(j)));
        }
      }
    ...
    _this.mod = function (e) {
      ...
      for (var i = 0; i < _this.getLength(); i += 1) n[i] = _this.getAt(i);
  ...
  function qr8BitByte(data) {
    var _mode = 4, _data = data, _bytes = (function () {
      var bytes = [];
      for (var i = 0; i < data.length; i += 1) {
        ...
        else { i++; var u = 0x10000 + (((c & 0x3ff) << 10) | (data.charCodeAt(i) & 0x3ff)); bytes.push(0xf0 | (u >> 18)); bytes.push(0x80 | ((u >> 12) & 0x3f)); bytes.push(0x80 | ((u >> 6) & 0x3f)); bytes.push(0x80 | (u & 0x3f)); }
      }
      return bytes;
    })();
    var _this = {};
  ...
  function byteArrayOutputStream() {
    var _bytes = [];
    return {
      writeByte: function (b) { _bytes.push(b & 0xff); },
      writeShort: function (i) { _bytes.push(i & 0xff); _bytes.push((i >>> 8) & 0xff); },
      writeBytes: function (b, off, len) { off = off || 0; len = len || b.length; for (var i = 0; i < len; i += 1) _bytes.push(b[i + off] & 0xff); },
      writeString: function (s) { for (var i = 0; i < s.length; i += 1) _bytes.push(s.charCodeAt(i) & 0xff); },
      toByteArray: function () { return _bytes; }
    };
  }

  function base64EncodeOutputStream() {
    var _buffer = 0, _buflen = 0, _length = 0, _base64 = '';
    var encode = function (n) { n = n & 0x3f; if (n < 26) return 0x41 + n; if (n < 52) return 0x61 + (n - 26); if (n < 62) return 0x30 + (n - 52); if (n == 62) return 0x2b; if (n == 63) return 0x2f; throw new Error('n:' + n); };
    var writeEncoded = function (b) { _base64 += String.fromCharCode(encode(b & 0x3f)); };
    return {
      writeByte: function (n) {
        _buffer = (_buffer << 8) | (n & 0xff); _buflen += 8; _length += 1;
        while (_buflen >= 6) { writeEncoded(_buffer >>> (_buflen - 6)); _buflen -= 6; }
      },
      flush: function () {
        if (_buflen > 0) { writeEncoded(_buffer << (6 - _buflen)); _buffer = 0; _buflen = 0; }
        if (_length % 3 != 0) { var padlen = 3 - _length % 3; for (var i = 0; i < padlen; i += 1) _base64 += '='; }
      },
      toString: function () { return _base64; }
    };
  }

  function gifImage(width, height) {
    var _width = width, _height = height, _data = new Array(width * height);
    var _this = {};
    _this.setPixel = function (x, y, pixel) { _data[y * _width + x] = pixel; };
    _this.write = function (out) {
      out.writeString('GIF87a');
      out.writeShort(_width); out.writeShort(_height);
      out.writeByte(0x80); out.writeByte(0); out.writeByte(0);
      out.writeByte(0); out.writeByte(0); out.writeByte(0);
      out.writeByte(0xff); out.writeByte(0xff); out.writeByte(0xff);
      out.writeString(','); out.writeShort(0); out.writeShort(0);
      out.writeShort(_width); out.writeShort(_height); out.writeByte(0);
      var lzwMinCodeSize = 2;
      var raster = getLZWRaster(lzwMinCodeSize);
      out.writeByte(lzwMinCodeSize);
      var offset = 0;
      while (raster.length - offset > 255) { out.writeByte(255); out.writeBytes(raster, offset, 255); offset += 255; }
      out.writeByte(raster.length - offset);
      out.writeBytes(raster, offset, raster.length - offset);
      out.writeByte(0x00); out.writeString(';');
    };
    var bitOutputStream = function (out) {
      var _out = out, _bitLength = 0, _bitBuffer = 0;
      return {
        ...
        flush: function () { if (_bitLength > 0) _out.writeByte(_bitBuffer); }
      };
    };
    var getLZWRaster = function (lzwMinCodeSize) {
      var clearCode = 1 << lzwMinCodeSize, endCode = (1 << lzwMinCodeSize) + 1;
      var bitLength = lzwMinCodeSize + 1;
      var table = lzwTable(); var i;
      for (i = 0; i < clearCode; i += 1) table.add(String.fromCharCode(i));
      table.add(String.fromCharCode(clearCode)); table.add(String.fromCharCode(endCode));
      var byteOut = byteArrayOutputStream(); var bitOut = bitOutputStream(byteOut);
      bitOut.write(clearCode, bitLength);
      var dataIndex = 0;
      var s = String.fromCharCode(_data[dataIndex]); dataIndex += 1;
      while (dataIndex < _data.length) {
        var c = String.fromCharCode(_data[dataIndex]); dataIndex += 1;
        if (table.contains(s + c)) { s = s + c; }
        else {
          bitOut.write(table.indexOf(s), bitLength);
          if (table.size() < 0xfff) { if (table.size() == (1 << bitLength)) bitLength += 1; table.add(s + c); }
          s = c;
        }
      }
      bitOut.write(table.indexOf(s), bitLength);
    ...
    var lzwTable = function () {
      var _map = {}, _size = 0;
      var _this = {};
      _this.add = function (key) { if (_this.contains(key)) throw new Error('dup key:' + key); _map[key] = _size; _size += 1; };
